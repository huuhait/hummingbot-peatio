...
class PeatioExchangeUnitTest(unittest.TestCase):
    ...
    def test_buy_and_sell(self):
        price = self.connector.get_price(self.trading_pair, True) * Decimal("1.4")
        price = self.connector.quantize_order_price(self.trading_pair, price)
        amount = self.connector.quantize_order_amount(self.trading_pair, Decimal("1.1"))
        ...
        self.assertEqual("ROGER", order_completed_event.base_asset)
        ...
        price = self.connector.get_price(self.trading_pair, True) * Decimal("0.6")
        price = self.connector.quantize_order_price(self.trading_pair, price)
        amount = self.connector.quantize_order_amount(self.trading_pair, Decimal("1.1"))
        ...
        maker_fee = self.connector.estimate_fee_pct(True)
        taker_fee = self.connector.estimate_fee_pct(False)
        expected_base_bal = base_bal - base_amount_traded
        expected_base_bal_with_fee_m = base_bal - (base_amount_traded * (Decimal("1") + maker_fee))
        expected_base_bal_with_fee_t = base_bal - (base_amount_traded * (Decimal("1") + taker_fee))
        self.ev_loop.run_until_complete(asyncio.sleep(6))
        self.ev_loop.run_until_complete(self.connector._update_balances())
        self.ev_loop.run_until_complete(asyncio.sleep(6))
        try:
            self.assertAlmostEqual(expected_base_bal_with_fee_t, self.connector.get_available_balance(self.base_token), 5)
        except Exception:
            try:
                self.assertAlmostEqual(expected_base_bal_with_fee_m, self.connector.get_available_balance(self.base_token), 5)
            except Exception:
    ...
    def test_limit_makers_unfilled(self):
        ...
        expected_quote_bal_with_fee_m = quote_bal - ((price * amount) * (Decimal("1") + maker_fee))
        expected_quote_bal_with_fee_t = quote_bal - ((price * amount) * (Decimal("1") + taker_fee))
        self.ev_loop.run_until_complete(asyncio.sleep(1))
        self.ev_loop.run_until_complete(self.connector._update_balances())
        self.ev_loop.run_until_complete(asyncio.sleep(2))
        ...
        try:
            ...
            try:
                self.assertAlmostEqual(expected_quote_bal_with_fee_m, self.connector.get_available_balance(self.quote_token), 5)
            except Exception:
    ...
    def test_order_quantized_values(self):
        ...
        self.assertGreater(self.connector.get_balance("ROGER"), Decimal("10"))
        self.assertGreater(self.connector.get_balance("BTC"), Decimal("0.00003"))
    ...
    def test_orders_saving_and_restoration(self):
        ...
        try:
            ...
            # Try to put limit buy order for 1.1 ROGER, and watch for order creation event.
